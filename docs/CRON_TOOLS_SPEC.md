...
The Linux user service does not create a separate cron unit. On Linux, `bestie service install` writes one `bestie.service` unit whose hidden foreground command, `bestie service run`, starts configured Telegram, Zalo, cron, and future service targets together. On macOS, `bestie service install` writes launchd LaunchAgents for the service runtime and Web UI. On Windows, `bestie service install` writes `Bestie.lnk` plus `Bestie.ico` into the user Startup folder so `bestie daemon start --channel all` runs at logon, including the cron daemon and Web UI companion daemon.
