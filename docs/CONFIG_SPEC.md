...
`customerMemory: "primary"`, `knowledgeAccess: "primary"`, and `toolPolicy: "allowlist"` require `allowUnsafeSharedData: true`; this is an explicit acknowledgement that shared private data or broad tools can be exposed. Do not use those options for normal customer support. They are not a prompt-security control and tool allowlisting is not namespace-aware yet.
