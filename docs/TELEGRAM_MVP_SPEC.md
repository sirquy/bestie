...
- Basic owner attachment handling: save supported Telegram file objects under `.bestie/workspace/media/inbound/...`, forward caption, metadata, local path, and a bounded preview for text, PDF, and DOCX files to the model, and treat uploaded content as untrusted. Full saved files may be read later through existing local read tools; other binary/media contents are not interpreted by the Telegram layer.
...
- `attachments` is optional. Defaults are `downloadPolicy: allow`, `maxBytes: 20971520`, `previewMaxBytes: 16384`, `parseMaxBytes: 5242880`, automatic `visionPolicy` based on whether the primary provider adapter supports image inputs, `visionMaxBytes: 4194304`, `transcriptionPolicy: deny`, `transcriptionMaxBytes: 10485760`, and no MIME or extension allowlist. Set `downloadPolicy` to `deny` to disable attachment downloads, lower `parseMaxBytes` to skip expensive text extraction for larger files, set `visionPolicy` to `deny` to force image inputs off, or set `transcriptionPolicy` to `allow` only when a top-level `transcription` provider is configured. OpenAI-compatible transcription requires `BESTIE_TRANSCRIPTION_API_KEY`; local-whisper requires the local binary and model file.
