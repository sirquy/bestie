...
- Hygiene score trend snapshots are persisted whenever `bestie memory hygiene status`, `bestie memory hygiene doctor`, or the matching Telegram/Zalo channel commands run. Status, trend, and doctor output report whether the current 0-100 hygiene score improved, regressed, or stayed flat compared with the previous snapshot. Weekly maintenance digests call `internal.memory_hygiene_trend` and `internal.plan_memory_hygiene` alongside `internal.plan_memory_rebalance` so the report can include recent score direction, cleanup candidates, and tier drift recommendations without mutating memory. The cron daemon also emits a one-time owner-channel regression alert per latest snapshot when the score falls below 60 or drops across three consecutive snapshots.
