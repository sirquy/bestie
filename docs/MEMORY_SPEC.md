...
- `src/memory/sqlite-store.ts` opens `.bestie/data/memory.sqlite`, applies schema, and can add/list active memories, persisted messages, rolling conversation summaries, pause state, pending memories, and pending action approvals.
- `bestie doctor` verifies the local memory database can be created and opened.
- `bestie memory status`, `bestie memory pause`, `bestie memory resume`, `bestie memory list [--scope core|project|session]`, `bestie memory tiers`, `bestie memory summary`, `bestie memory summaries [--channel <name>] [--user <id>] [--limit <n>] [--json]`, `bestie memory summaries refresh [--channel terminal|telegram|zalo|ui] [--user <id>] [--limit <n>] [--json]`, `bestie memory rebalance [--dry-run|--apply] [--yes] [--json]`, `bestie memory search <query>`, `bestie memory analyze [--mode all|duplicates|stale|conflicts] [--json]`, `bestie memory hygiene [status|trend|doctor|--apply] [--fix] [--yes] [--json]`, `bestie memory digest, bestie memory cleanup --dry-run|--apply [--yes] [--json]`, `bestie memory governance status|policy full|governed`, `bestie memory pin <id>`, `bestie memory unpin <id>`, `bestie memory move <id> --scope core|project|session`, `bestie memory supersede <oldId> <newId>`, `bestie memory maintenance install|status|remove [--channel telegram:<id>|zalo:<id>] [--schedule <cron>]`, `bestie memory add <type> <content>`, `bestie memory inspect <id>`, `bestie memory edit <id> <content>`, `bestie memory forget <id>`, `bestie memory messages [--limit <n>] [--role user|assistant|system]`, `bestie memory messages search <query> [--limit <n>] [--role user|assistant|system]`, `bestie memory export`, `bestie memory clear --yes`, `bestie memory pending [--limit <n>]`, `bestie memory pending search <query> [--limit <n>]`, `bestie memory pending inspect <id>`, `bestie memory approve <id>`, `bestie memory reject <id>`, and `bestie memory reject-all --yes` support manual inspection, clearer active-memory listing, rolling conversation summary inspection and on-demand refresh, continuity counts in the main memory summary, Web UI memory dashboard, and Doctor diagnostics for long threads missing/stale summaries, tier dashboard, read-only tier rebalance recommendations, policy-gated tier rebalance application for non-review-only moves, tier filtering and movement, supersession marking, simple active-memory search, structured duplicate/stale/conflict analysis, readable hygiene dry-runs, concise hygiene status, direct hygiene trend with policy-aware next commands and a 0-100 hygiene score, read-only hygiene doctor checks with the same score, safe hygiene doctor fixes for maintenance digest installation, policy-aware hygiene apply, retrieval policy inspection and switching, pinned-memory curation, policy-aware duplicate/stale cleanup planning and application, weekly cron-backed memory maintenance reports to the owner channel, per-memory metadata inspection, pause/resume control, safe manual writes, manual edits, soft deletion, bounded persisted-message inspection, search, and role filtering, JSON export, confirmed clearing, and bounded sensitive-memory inspection, search, approval, rejection, and confirmed bulk rejection with next-command hints.
- Chat reads approved active memories as context, supports `/status`, `/memory`, `/memory tiers`, `/memory summary`, `/memory rebalance dry-run`, `/memory rebalance apply`, `/memory rebalance apply confirm`, `/memory scope core|project|session`, `/memory inspect <id>`, `/memory move <id> core|project|session`, `/memory supersede <oldId> <newId>`, `/memory analyze`, `/memory hygiene`, `/memory hygiene status`, `/memory hygiene trend`, `/memory hygiene doctor`, `/memory hygiene apply`, `/memory hygiene apply confirm`, `/memory digest, /memory cleanup dry-run`, `/memory governance status`, `/memory governance policy full|governed`, `/memory pin <id>`, `/memory unpin <id>`, `/memory maintenance install|status|remove`, `/memory pause`, `/memory resume`, and `/pending` for quick in-chat local memory control, tier dashboard, read-only tier rebalance recommendations, policy-gated tier rebalance application for non-review-only moves, tier filtering and movement, active-memory metadata inspection, supersession marking, cleanup planning, readable hygiene dry-runs, concise hygiene status, direct hygiene trend, read-only hygiene doctor checks, policy-aware hygiene apply, governance inspection, retrieval policy switching, pinned-memory curation, and cron-backed maintenance report management, and persists successful terminal/Telegram/Zalo user/assistant messages only while memory is active. Provider context includes the last 120 persisted messages by default for the current channel/user plus a rolling conversation summary for older turns once a terminal, Telegram, Zalo, or Web UI thread exceeds that window, so short-term continuity can survive longer conversations, UI sessions, and terminal restarts without injecting unbounded transcripts. `memory.recentMessageLimit` can tune that recent-message window per install; the runtime clamps extreme values conservatively. Web UI chat also falls back to persisted session messages when a request omits or sends an empty history array, preserving continuity across reloads and alternate clients. `memory.retrievalPolicy` defaults to `full`, which injects every active memory in core/project/session order; `governed` still injects every active memory but promotes pinned/current/high-confidence memories and labels low-confidence, expired, scoped, or superseded memories. The tool loop exposes `internal.list_memories` for complete active-memory listing, `internal.search_memories` for model-requested active-memory lookup, `internal.analyze_memories` for duplicate/stale/conflict cleanup planning, `internal.plan_memory_rebalance` for tier drift checks, and `internal.remember_memory` for model-requested writes without language-specific keyword detection; writes are stored, queued, or denied according to `memory.writePolicy`, memory pause state, and memory safety policy. Telegram `ask` writes create an approval request with inline Approve/Deny buttons; approving stores the pending memory and denying rejects it.
...
- Hygiene score trend snapshots are persisted whenever `bestie memory hygiene status`, `bestie memory hygiene doctor`, or the matching Telegram/Zalo channel commands run. Status, trend, and doctor output report whether the current 0-100 hygiene score improved, regressed, or stayed flat compared with the previous snapshot. Weekly maintenance digests refresh stale rolling conversation summaries before calling `internal.memory_hygiene_trend` and `internal.plan_memory_hygiene` alongside `internal.plan_memory_rebalance`, so the report can include continuity upkeep, recent score direction, cleanup candidates, and tier drift recommendations without mutating active memories. The cron daemon also emits a one-time owner-channel regression alert per latest snapshot when the score falls below 60 or drops across three consecutive snapshots.
...
- install, inspect, and remove a cron-backed weekly memory hygiene digest that reports `deleteIds`, `reviewOnlyIds`, duplicate groups, stale memories, conflicts, and safe apply commands without deleting anything
- save hygiene score snapshots from status/doctor runs and report score movement since the previous snapshot
- export memory
- clear memory
- `forget this`
- pause memory
