...
process.stdin.on("data", (chunk) => {
  for (const line of chunk.trim().split(/\\r?\\n/)) {
    ...
    if (request.method === "tools/list") {
      process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { tools: [{ name: "read_file", description: "Read a local file" }] } }) + "\\n");
    }
...
async function runCli(args, input = undefined) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(process.execPath, [cliPath, ...args], { cwd: rootDir, stdio: ["pipe", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
