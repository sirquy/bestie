...
export function formatToolActivityLabel(request: AgentToolRequest): string {
  ...
  if (request.tool === "internal.git_diff") {
    return booleanArg(args.staged) ? "staged git diff" : "git diff";
  }
...
export async function runAgentToolRequest(options: RunAgentToolRequestOptions): Promise<McpToolCallResult> {
  ...
  if (options.request.tool === "internal.git_status") {
    const result = await readGitStatusTool({ paths: options.paths, approver: options.approver, policy: options.policy });
  ...
  if (options.request.tool === "internal.git_diff") {
    const result = await readGitDiffTool({ paths: options.paths, staged: booleanArg(args.staged), maxBytes: numberArg(args.maxBytes), approver: options.approver, policy: options.policy });
  ...
  if (options.request.tool === "internal.git_log") {
    const result = await readGitLogTool({ paths: options.paths, limit: numberArg(args.limit), approver: options.approver, policy: options.policy });
    return { ok: result.allowed, status: result.allowed ? "pass" : "fail", message: result.reason, result: { output: result.output } };
  }
