...
export async function runTunnelTool(options: { action: TunnelToolAction; config: AppConfig; paths: RuntimePaths; clientVersion: string; approver?: PermissionApprover; policy?: PermissionPolicy }) {
  ...
  if (configured !== "allow") {
    const permission = await reviewActionPermission({ category, action: `internal.tunnel_${options.action}`, target: "Bestie remote-access tunnel", reason: `Agent requested tunnel ${options.action}.`, trusted: options.action === "status" }, { paths: options.paths, approver: options.approver, policy: options.policy });
