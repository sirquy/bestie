...
type ProviderId = "gemini-cli" | "claude-cli" | "codex-cli" | "anthropic" | "openai" | "groq" | "openrouter" | "custom-openai" | "custom-anthropic" | "ollama" | "gemini" | "antigravity";
...
export async function runLlmSetupCommand(options: LlmSetupOptions = {}): Promise<void> {
  ...
  try {
    ...
    writeLine(keyValue("Base URL", candidate.baseUrl ?? (candidate.mode === "local" ? "local" : "SDK default")));
...
async function collectLlmSetupSelection(questioner: Pick<Questioner, "ask" | "askHidden" | "select">, providerFlag: string | undefined, writeLine: (message: string) => void): Promise<LlmSetupSelection> {
  ...
  const baseUrl = catalog.runtimeProvider === "claude-cli" || catalog.runtimeProvider === "codex-cli" || catalog.runtimeProvider === "gemini-cli" || catalog.runtimeProvider === "gemini" ? undefined : await askNonEmpty(questioner.ask, "Base URL", catalog.defaultBaseUrl);
...
function providerAliases(id: ProviderId): string[] {
  if (id === "gemini-cli") return ["geminicli", "gemini-code"];
