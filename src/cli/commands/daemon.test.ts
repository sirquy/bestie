...
test("runDaemonCommand starts, reports, and stops the daemon", async () => {
  ...
  try {
    ...
    const state = JSON.parse(await readFile(resolve(paths.appDir, "daemon.json"), "utf8")) as { pid: number; args: string[]; logPath: string };
    assert.equal(state.pid, 4242);
    assert.deepEqual(state.args.slice(-1), ["telegram"]);
    assert.equal(state.logPath, resolve(paths.logsDir, "daemon.log"));
    assert.match(output.join("\n"), /Daemon started with pid 4242/);
    ...
    assert.match(output.at(-2) ?? "", /running with pid 4242/);
    ...
    assert.match(output.at(-1) ?? "", /Daemon stopped: 4242/);
  } finally {
...
test("runDaemonCommand does not kill a reused stale daemon pid", async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      `${JSON.stringify({ pid: 4242, command: process.execPath, args: ["/old/bestie", "telegram"], startedAt: new Date().toISOString(), logPath: resolve(paths.logsDir, "daemon.log") }, null, 2)}\n`,
      { mode: 0o600 },
    );
    ...
    const state = JSON.parse(await readFile(resolve(paths.appDir, "daemon.json"), "utf8")) as { pid: number };
    assert.equal(state.pid, 4343);
