const SECRET_LIKE_PATTERN = /(?:sk-[A-Za-z0-9_-]{8,}|test-key|telegram-token|bot-token|\b(?=[A-Za-z0-9]{32,}\b)(?=[A-Za-z0-9]*\d)[A-Za-z0-9]+\b|Bearer\s+[A-Za-z0-9._~+/=-]+|("?(?:api[_-]?key|token|secret|password)"?\s*[:=]\s*)"?[^",\s}]+"?)/gi;
