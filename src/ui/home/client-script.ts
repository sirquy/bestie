...
function renderSkillLibrary(library) {
  ...
  const sources = ["all", ...new Set((state.skillLibrary ?? []).map((skill) => skill.sourceId ?? "remote-official"))];
...
function previewLibrarySkill(name, sourceId) {
  if (!name) return;
  fetch("/api/skills/library/item?name=" + encodeURIComponent(name) + "&sourceId=" + encodeURIComponent(sourceId ?? "remote-official"))
    .then((response) => response.json())
    .then((result) => {
      showSkillEditorPage();
