      ...
      <main>
        <section class="topbar">
          <div>
            ...
            <h1>Bestie Console</h1>
          </div>
          <div class="runtime-card" id="runtime-card">
            <div class="label">${icon("activity")} Runtime</div>
            <div class="value">Loading...</div>
          </div>
        </section>
        <section class="metric-grid" id="status-grid" aria-live="polite">
          <article class="metric-card"><div class="label">Status</div><div class="value">Loading...</div></article>
        </section>
        <section class="panel-grid" data-active-panel="doctor-panel">
...
function renderPanel(panel: (typeof PANELS)[number]): string {
  return `<section class="panel${panel.id === "doctor-panel" ? " active" : ""}" id="${panel.id}" aria-live="polite" data-panel="${panel.id}">
