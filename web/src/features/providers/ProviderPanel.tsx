...
export function ProviderPanel({ data, loading, view, onData, onLoading }: ProviderPanelProps): ReactElement {
  ...
  const isGeminiCli = normalizedProvider === "gemini-cli";
  const isClaudeCli = normalizedProvider === "claude-cli";
  const isCodexCli = normalizedProvider === "codex-cli";
  const hidesBaseUrl = isGemini || isGeminiCli || isClaudeCli || isCodexCli;
  const isLocal = form.mode === "local" || normalizedProvider === "ollama" || isGeminiCli || isClaudeCli || isCodexCli;
